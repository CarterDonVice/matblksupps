  ...
  return (
    <div className="space-y-4 sm:space-y-5">
      ...
      <div className="relative">
        ...
        {/* Desktop: thumbnail-driven, also keep arrows for mobile screen-readers */}
        <button
          type="button"
          aria-label="Previous image"
          onClick={() => scrollToIndex(Math.max(0, activeIndex - 1))}
          disabled={activeIndex === 0}
          className="hidden sm:inline-flex absolute left-2 top-1/2 -translate-y-1/2 h-10 w-10 items-center justify-center rounded-full bg-ink/70 backdrop-blur text-bone hover:text-white border border-ink-600 disabled:opacity-30 disabled:cursor-not-allowed transition-opacity"
        >
        ...
        <button
          ...
          className="hidden sm:inline-flex absolute right-2 top-1/2 -translate-y-1/2 h-10 w-10 items-center justify-center rounded-full bg-ink/70 backdrop-blur text-bone hover:text-white border border-ink-600 disabled:opacity-30 disabled:cursor-not-allowed transition-opacity"
        >
          <ChevronRight className="h-5 w-5" strokeWidth={1.5} />
        </button>
      </div>

      {/* Thumbnail strip — clickable on all sizes, primary nav on desktop */}
          ...
          <button
            ...
            className={[
              ...
              activeIndex === i
                ...
                : 'border-ink-600 opacity-60 hover:opacity-90',
            ].join(' ')}
          >
