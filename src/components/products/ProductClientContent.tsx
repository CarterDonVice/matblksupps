          ...
          {relatedFlavors.length > 0 && (
            <div>
              <p className="text-sm font-medium text-foreground mb-2">
                Flavor: <span className="text-primary font-bold">{product.flavor}</span>
              </p>
              <div className="flex flex-wrap gap-2">
                ...
                {relatedFlavors.map(flavor => (
                  <Link href={`/product/${flavor.slug}`} key={flavor.id}>
                    <Badge variant="outline" className="text-lg py-1 px-3 cursor-pointer hover:bg-accent hover:text-accent-foreground">
                      {flavor.flavor}
                    </Badge>
                  </Link>
                ))}
            ...
            <div className="bg-card p-6 rounded-lg border border-border/50">
              <ul className="space-y-2 text-muted-foreground columns-1 sm:columns-2">
                {product.ingredients.map((ingredient, idx) => {
                  const isHeader =
                    !ingredient.startsWith('L-') &&
                    !ingredient.startsWith('Alpha-GPC') &&
                    !ingredient.startsWith('Huperzine') &&
                    !ingredient.startsWith('Caffeine') &&
                    !ingredient.startsWith('Dicaffeine') &&
                    !ingredient.startsWith('Total') &&
                    !ingredient.startsWith('Taurine') &&
                    !ingredient.startsWith('Himalayan') &&
                    !ingredient.startsWith('Potassium') &&
                    !ingredient.startsWith('Magnesium') &&
                    !ingredient.startsWith('Piperine') &&
                    !ingredient.startsWith('DMAE') &&
                    !ingredient.startsWith('Yohimbine') &&
                    !ingredient.startsWith('Hordenine') &&
                    !ingredient.startsWith('Rauwolscine') &&
                    !ingredient.startsWith('GlycerPump') &&
                    !ingredient.startsWith('Nitrosigine') &&
                    !ingredient.startsWith('Lion') &&
                    !ingredient.startsWith('Creatine') &&
                    !ingredient.startsWith('Betaine') &&
                    !ingredient.startsWith('Astragin') &&
                    !ingredient.startsWith('Whey') &&
                    !ingredient.startsWith('Digestive');

                  return (
                    <li
                      key={idx}
                      className={`${isHeader ? 'font-bold text-foreground pt-2 text-lg' : 'ml-4'} break-inside-avoid-column`}
                    >
                      {ingredient}
                    </li>
                  );
